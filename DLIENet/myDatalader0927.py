...
class ImageDataset(Dataset):
    def __init__(self, opt, unaligned=False, transforms_=None):
        ...
        self.root_a=opt.dataroot
        # if opt.mode=='train' :
        #     self.files_A = sorted(glob.glob(opt.dataroot + '/train3/trainA' + '/*.png'),key=lambda x:eval(os.path.basename(x).split(".")[0]))
        #     #self.files_A = sorted(glob.glob(opt.dataroot + '/train2/trainA' + '/*.png'))
        #     #print(self.files_A)
        #     self.files_B = sorted(glob.glob(opt.dataroot + '/train3/trainB' + '/*.png'),key=lambda x:eval(os.path.basename(x).split(".")[0]))
        #     #self.files_B = sorted(glob.glob(opt.dataroot + '/train2/trainB' + '/*.png'))
        if opt.mode=='train' :
            # self.files_A = sorted(glob.glob(opt.dataroot + '/train3/trainA' + '/*.png'),key=lambda x:eval(os.path.basename(x).split(".")[0]))
            self.files_A = sorted(glob.glob(opt.dataroot + '/train2/trainA' + '/*.png'))
            # print(self.files_A)
            ...
            self.files_A = sorted(glob.glob(opt.dataroot + '/test/testA' + '/*.png'))
            self.files_B = sorted(glob.glob(opt.dataroot + '/test/testB' + '/*.png'))
        else:
            self.files_A = sorted(glob.glob(opt.dataroot + '/test2/testA' + '/*.png'),key=lambda x:eval(os.path.basename(x).split(".")[0]))
            self.files_B = sorted(glob.glob(opt.dataroot + '/test2/testB' + '/*.png'),key=lambda x:eval(os.path.basename(x).split(".")[0]))
...
def cfg():
    ...
    parse.add_argument('--mode', type=str, default='train', help='number of epochs of training')
    opt = parse.parse_args()
    return opt
def train_data_loader(arg):
    train_data_loader = DataLoader(ImageDataset(arg,unaligned=False),batch_size=arg.batchSize, shuffle=False, pin_memory=True, drop_last=True)
